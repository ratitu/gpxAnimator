...
uploaded_photos = st.file_uploader("Upload Photos (with EXIF info)", type=["jpg", "jpeg", "png"], accept_multiple_files=True)
...
def create_animation(points, fps, duration, size, zoom, color, follow, url_template, photos, photo_dur):
    ...
    for photo in photos:
        ...
        if photo["timestamp"]:
            ...
            for i, pt in enumerate(anim_points):
                if pt["time"]:
                    diff = abs((pt["time"].replace(tzinfo=None) - photo_ts).total_seconds())
        ...
        if best_frame_idx == -1 and photo["lat"] is not None and photo["lon"] is not None:
            # Fallback to Match by Location
            min_dist = float('inf')
            for i, pt in enumerate(anim_points):
                dist = np.sqrt((photo["lat"] - pt["lat"])**2 + (photo["lon"] - pt["lon"])**2)
                if dist < min_dist:
                    min_dist = dist
                    best_frame_idx = i
            # Ignore if too far
    ...
    for i in range(len(anim_points)):
        ...
        if follow:
            image = frame_map.render(zoom=zoom, center=(current_pt["lon"], current_pt["lat"]))
        else:
        ...
        if active_candidates:
            ...
            border = 5
            framed_photo = Image.new("RGB", (photo_img.width + 2*border, photo_img.height + 2*border), "white")
            framed_photo.paste(photo_img, (border, border))
            image.paste(framed_photo, (size - framed_photo.width - 15, size - framed_photo.height - 15))
        ...
        if st.button("Generate Video"):
            ...
            with st.spinner("Generating animation..."):
                video_path = create_animation(
                    points, fps, duration_target, map_size, zoom_level, line_color, follow_mode, map_url, photos, photo_display_duration
                )
                
                if video_path:
                    st.video(video_path)
                    with open(video_path, "rb") as f:
                        st.download_button("Download Video", f, "gpx_animation.mp4", "video/mp4")
